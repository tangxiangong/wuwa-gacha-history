...
> 注意：实际游戏请求中**没有** `size` / `lastId` 字段。服务端一次性返回该卡池自建号以来的全部抽卡记录（本 repo 早期文档和代码里保留的这两个字段是错误的，已移除）。
...
限定池（`cardPoolType = 1, 2`）的 `cardPoolId` 随每期活动变化，需要每次从游戏 URL 或抓包中实时获取。常驻及新手池的 ID 是固定的，见仓库根目录的 `card_pool_id.json`。
...
| Parameter    | Type   | Description                                                          |
| :----------- | :----- | :------------------------------------------------------------------- |
| cardPoolType | string | **本地化中文字符串**（例如 "角色精准调谐"），不是请求中的整数         |
