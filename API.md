...
- 响应**没有 `id` 字段**，无法用服务端稳定 ID 去重。
- 响应顺序就是游戏 UI 里从上到下的顺序：跨时间戳是**新时间在前**，同一秒内是**先抽的在前**（例如一次 10 连最后出的 5★ 位于该批的末尾）。
- 存库策略：按 `cardPoolType` 做「整池删除 + 重新写入」（响应已是该池完整的抽卡历史），并引入 `seq INTEGER` 列记录响应下标（`seq = 0` 即响应第一条 = 游戏顶部那条）。
- 查询用 `ORDER BY time DESC, seq ASC`，从而**严格还原游戏内顺序**——相同时间戳时必须用 `seq` 做 tie-breaker，不能依赖 SQLite 的非稳定 `time` 排序。
- 合成主键 `record_id = "{cardPoolType}-{seq}"`，纯内部字段，仅用于主键唯一约束。
