...
def fetch_list(catalogue_id: int, limit: int = 1000) -> list[dict]:
    body = urllib.parse.urlencode(
        {"catalogueId": catalogue_id, "page": 1, "limit": limit}
    ).encode()
    headers = {
        **COMMON_HEADERS,
...
def run(out_dir: Path, only: str | None, concurrency: int) -> int:
    ...
    for cat in categories:
        if cat not in CATALOGUES:
            print(f"unknown category: {cat}; choose from {list(CATALOGUES)}", file=sys.stderr)
            errors += 1
            continue
        cid = CATALOGUES[cat]
        print(f"\n=== {cat} (catalogueId={cid}) ===")
        try:
            records = fetch_list(cid)
        ...
        with cf.ThreadPoolExecutor(max_workers=concurrency) as pool:
            futures = {pool.submit(download, url, dest): (name, dest) for name, url, dest in tasks}
        ...
        print(f"  summary: downloaded={counts['ok']} cached={counts['skip']} failed={counts['err']}")
    return errors
